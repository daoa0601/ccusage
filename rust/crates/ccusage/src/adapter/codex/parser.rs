...
#[derive(Default)]
struct PendingFallbackEvents {
    events: Vec<CodexTokenUsageEvent>,
}

impl PendingFallbackEvents {
    fn push(&mut self, event: CodexTokenUsageEvent) {
    ...
    fn flush(&mut self, visit: &mut impl FnMut(CodexTokenUsageEvent) -> Result<()>) -> Result<()> {
...
fn visit_codex_session_entry(
    ...
    pending_fallback_events: &mut PendingFallbackEvents,
    visit: &mut impl FnMut(CodexTokenUsageEvent) -> Result<()>,
) -> Result<()> {
    ...
    if entry_type == Some("turn_context") {
        if let Some(model) = value.payload.as_ref().and_then(codex_model_from_payload) {
            ...
            *current_model_is_fallback = false;
            pending_fallback_events.resolve(current_model.as_deref().unwrap_or_default(), visit)?;
        }
        return Ok(());
    }
    ...
    if let Some(model) = parsed_model.clone() {
        pending_fallback_events.resolve(&model, visit)?;
        *current_model = Some(model);
        *current_model_is_fallback = false;
    }
