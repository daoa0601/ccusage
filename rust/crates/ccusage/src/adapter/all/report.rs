...
pub(super) fn report_json(rows: &[AllRow], kind: AgentReportKind) -> Value {
    json!({
        rows_key(kind): rows.iter().map(|row| row_json(row, kind)).collect::<Vec<_>>(),
        "totals": totals_json(rows),
        "totalsBySource": totals_by_source_json(rows),
    })
}

fn row_json(row: &AllRow, kind: AgentReportKind) -> Value {
    let mut value = json!({
        "period": row.period,
        "agent": row.agent,
        "source": row.agent,
    ...
    if let Some(obj) = value.as_object_mut() {
        match kind {
            AgentReportKind::Daily => {
                obj.insert("date".to_string(), json!(row.period));
            }
            AgentReportKind::Weekly => {
                obj.insert("week".to_string(), json!(row.period));
            }
            AgentReportKind::Monthly => {
                obj.insert("month".to_string(), json!(row.period));
            }
            AgentReportKind::Session => {
                obj.insert("sessionId".to_string(), json!(row.period));
            }
        }
    }
    value
...
fn totals_by_source_json(rows: &[AllRow]) -> Value {
    let mut totals = BTreeMap::<&'static str, SourceTotals>::new();
    for row in rows {
        if let Some(agent_breakdowns) = row.agent_breakdowns.as_ref() {
            for breakdown in agent_breakdowns {
                totals.entry(breakdown.agent).or_default().add(breakdown);
            }
            continue;
        }
        totals.entry(row.agent).or_default().add(row);
    }
    Value::Object(
        totals
            .into_iter()
            .map(|(source, totals)| (source.to_string(), totals.into_json()))
            .collect(),
    )
}
