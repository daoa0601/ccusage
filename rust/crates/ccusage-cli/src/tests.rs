...
#[test]
fn parses_legacy_tool_all_as_all_agent_report() {
    let cli = parse(&["ccusage", "daily", "--tool", "all", "--json"]);
    let Some(Command::All(args)) = cli.command else {
        panic!("expected all-agent command");
    };
    assert_eq!(args.kind, AgentReportKind::Daily);
...
#[test]
fn parses_legacy_tool_subset_for_all_agent_report() {
    let cli = parse(&["ccusage", "monthly", "--tool", "codex,claude,codex"]);
    let Some(Command::All(args)) = cli.command else {
        panic!("expected all-agent command");
    };
    assert_eq!(args.kind, AgentReportKind::Monthly);
    assert_eq!(
        args.shared.tool_filter,
        Some(vec!["codex".to_string(), "claude".to_string()])
    );
}

#[test]
fn parses_legacy_by_model_for_all_agent_report() {
    let cli = parse(&["ccusage", "session", "--tool", "pi", "--by-model"]);
    let Some(Command::All(args)) = cli.command else {
        panic!("expected all-agent command");
    };
    assert_eq!(args.kind, AgentReportKind::Session);
