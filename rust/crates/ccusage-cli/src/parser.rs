...
pub fn parse_tool_filter(value: &str) -> Result<Option<Vec<String>>, String> {
    let tools = value
        ...
        .collect::<Vec<_>>();
    if tools.is_empty() {
        return Err("`--tool` requires at least one tool name".to_string());
    }
    if tools.iter().any(|tool| tool == "all") {
        if tools.len() == 1 {
            return Ok(None);
        }
        return Err("`all` cannot be combined with other tool names".to_string());
    }

    let mut selected = Vec::new();
    for tool in tools {
        if !TOOL_FILTER_AGENTS.contains(&tool.as_str()) {
            return Err(format!(
                "Unsupported tool \"{tool}\". Expected one of: {}.",
                ["all"]
                    .into_iter()
                    .chain(TOOL_FILTER_AGENTS.iter().copied())
                    .collect::<Vec<_>>()
                    .join(", ")
            ));
        }
